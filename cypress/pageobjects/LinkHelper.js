...
class LinkHelper {
  ...
  linkExists() {
    ...
    // Add debugging to see what href is actually found
    cy.get(`@link-${this.#linkNumber}`).then(($link) => {
      const actualHref = $link.attr('href');
      cy.log(`Expected href: ${this.#href}`);
      cy.log(`Actual href: ${actualHref}`);
    });
