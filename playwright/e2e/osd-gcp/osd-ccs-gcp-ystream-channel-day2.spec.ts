...
const clusterProfiles = require('../../fixtures/osd-gcp/osd-ccs-gcp-ystream-channel.spec.json');
...
/** Cluster from Day-1 create (osd-ccs-gcp-ystream-channel-day1.spec.ts) or env override. */
const clusterName =
...
test.describe.serial(
  `OSD GCP CCS Y-stream Channel Day-2 overview and edit (${clusterName})`,
  { tag: ['@advanced', '@ystream', '@osd', '@gcp', '@day2'] },
  () => {
    ...
    test('Navigate to OSD GCP cluster overview', async ({
      clusterListPage,
      clusterDetailsPage,
    }) => {
    ...
    test('Overview does not show Channel group when Y-stream channel is enabled', async ({
      ...
      await expect(clusterDetailsPage.overviewChannelGroup()).toBeVisible();
    });
    ...
    test('Overview Channel row shows value, hint, and Learn more link', async ({
      ...
      expect(channelText).toMatch(/stable-4\.\d+|fast-4\.\d+|eus-4\.\d+|N\/A/);
    ...
    test('Save updates on Overview page Channel row', async ({ clusterDetailsPage }) => {
      ...
      await clusterDetailsPage.editChannelModalSaveButton().click();
      await expect(clusterDetailsPage.editChannelModal()).not.toBeVisible({ timeout: 30000 });
      await expect(clusterDetailsPage.overviewChannelValue()).toContainText(yStream.SelectedChannel);
    });
