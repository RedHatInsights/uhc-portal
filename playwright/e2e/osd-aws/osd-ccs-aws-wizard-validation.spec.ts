...
test.describe.serial(
  ...
  () => {
    ...
    test(`Cloud provider field validations`, async () => {
      ...
      if (awsSecretKey) {
        await osdWizardPage.awsSecretKeyInput().fill(awsSecretKey);
      }
      await osdWizardPage.wizardNextButton().click();
      await osdWizardPage.isTextContainsInPage(
      ...
      if (awsAccessKey) {
        await osdWizardPage.awsAccessKeyInput().fill(awsAccessKey);
        await osdWizardPage.awsAccessKeyInput().clear();
      }
      await osdWizardPage.wizardNextButton().click();
      await osdWizardPage.isTextContainsInPage(
      ...
      await osdWizardPage.awsAccountIDInput().clear();
      await osdWizardPage
        .awsAccountIDInput()
        .fill(ClustersValidation.ClusterSettings.CloudProvider.AWS.InValidAWSAccountIdValues[0]);
      await osdWizardPage.awsAccountIDInput().blur();
      await osdWizardPage.isTextContainsInPage(
      ...
      await osdWizardPage
        .awsAccountIDInput()
        .fill(ClustersValidation.ClusterSettings.CloudProvider.AWS.InValidAWSAccountIdValues[1]);
      await osdWizardPage.awsAccountIDInput().blur();
      await osdWizardPage.wizardNextButton().click();
      await osdWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.CloudProvider.AWS.InValidAWSAccountIdError,
      ...
      if (awsAccountID && awsAccessKey && awsSecretKey) {
        await osdWizardPage.awsAccountIDInput().clear();
        await osdWizardPage.awsAccountIDInput().fill(awsAccountID);
        await osdWizardPage.awsAccessKeyInput().clear();
        await osdWizardPage.awsAccessKeyInput().fill(awsAccessKey);
        await osdWizardPage.awsSecretKeyInput().clear();
        await osdWizardPage.awsSecretKeyInput().fill(awsSecretKey);
      }
    ...
    test(`Cluster details field validations`, async () => {
      ...
      if (clusterProperties.CloudProvider.includes('GCP')) {
        ...
        await osdWizardPage.advancedEncryptionLink().click();
        await osdWizardPage.useCustomKMSKeyRadio().check();
        await osdWizardPage.wizardNextButton().click();
        await osdWizardPage.isTextContainsInPage(
          ClustersValidation.ClusterSettings.Details.AWS.KeyARNs.EmptyValueError,
        );

        await osdWizardPage.keyArnInput().clear();
        await osdWizardPage
          .keyArnInput()
          .fill(ClustersValidation.ClusterSettings.Details.AWS.KeyARNs.WrongFormatValue);
        await osdWizardPage.wizardNextButton().click();
        await osdWizardPage.isTextContainsInPage(
        ...
        await osdWizardPage
          .keyArnInput()
          .fill(ClustersValidation.ClusterSettings.Details.AWS.KeyARNs.WrongFormatWithWhitespace);
        await osdWizardPage.wizardNextButton().click();
        await osdWizardPage.isTextContainsInPage(
          ClustersValidation.ClusterSettings.Details.AWS.KeyARNs.WrongFormatWithWhitespaceError,
        );

        await osdWizardPage.useDefaultKMSKeyRadio().check();
      }
      await osdWizardPage.wizardNextButton().click();
    });
    ...
    test(`Cluster autoscaling validations`, async () => {
      ...
      await osdWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.Machinepool.Common.ClusterAutoscaling.RequiredFieldError,
      );
      ...
      await osdWizardPage.clusterAutoscalingMemoryTotalMinInput().blur();
      await osdWizardPage.isTextContainsInPage(
        ClustersValidation.ClusterSettings.Machinepool.Common.ClusterAutoscaling.NegativeValueError,
      );
      ...
      await osdWizardPage.clusterAutoscalingScaleDownUtilizationThresholdInput().press('Control+a');
      ...
      await osdWizardPage.clusterAutoscalingScaleDownDelayAfterFailureInput().press('Control+a');
