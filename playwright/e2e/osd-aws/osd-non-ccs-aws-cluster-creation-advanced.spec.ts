...
test.describe.serial(
  ...
  () => {
    ...
    test(`OSD ${clusterProperties.CloudProvider} wizard - Networking configuration - CIDR definitions`, async ({
      ...
      await expect(createOSDWizardPage.machineCIDRInput()).toHaveValue(cidrRanges.MachineCIDR);
      await expect(createOSDWizardPage.serviceCIDRInput()).toHaveValue(cidrRanges.ServiceCIDR);
      await expect(createOSDWizardPage.podCIDRInput()).toHaveValue(cidrRanges.PodCIDR);
      await expect(createOSDWizardPage.hostPrefixInput()).toHaveValue(cidrRanges.HostPrefix);
      await page.locator(createOSDWizardPage.primaryButton).click();
    });
    ...
    test(`OSD ${clusterProperties.CloudProvider} wizard - Review and create page and its definitions`, async ({
      ...
      await expect(createOSDWizardPage.machineCIDRValue()).toContainText(cidrRanges.MachineCIDR);
      await expect(createOSDWizardPage.serviceCIDRValue()).toContainText(cidrRanges.ServiceCIDR);
      await expect(createOSDWizardPage.podCIDRValue()).toContainText(cidrRanges.PodCIDR);
      await expect(createOSDWizardPage.hostPrefixValue()).toContainText(cidrRanges.HostPrefix);
