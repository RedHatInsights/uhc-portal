...
const clusterProfiles = require('../../fixtures/rosa/rosa-cluster-classic-private-creation-advanced.spec.json');
const clusterProperties = clusterProfiles['rosa-classic-private-advanced'];

test.describe.serial(
  'ROSA cluster creation multizone private advanced settings',
  { tag: ['@day1', '@rosa', '@rosa-classic', '@private', '@multi-zone', '@advanced'] },
  () => {
    const region = process.env.QE_AWS_REGION || clusterProperties.Region.split(',')[0];
    const version = clusterProperties.Version || process.env.VERSION || '';
    ...
    test('Step - Cluster Settings - machine pool - Select advanced options', async ({
      ...
      await expect(page.getByText('x 3 zones = 6')).toBeVisible();
      await createRosaWizardPage.setMaximumNodeCount(
        clusterProperties.MachinePools[0].MaximumNodeCount,
      );
      await expect(page.getByText('x 3 zones = 9')).toBeVisible();
    ...
    test('Step - Networking - Configuration settings', async ({ createRosaWizardPage }) => {
      ...
      await expect(createRosaWizardPage.usePrivateLinkCheckbox()).toBeChecked();
