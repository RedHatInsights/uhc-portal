...
let sharedPage: Page;
let osdProductPage: OsdProductPage;

test.describe.serial(
  'OCP-79051 - OSD product page. Test is checking all main elements and text content on the page',
  { tag: ['@smoke', '@osd', '@overview'] },
  () => {
    test.beforeAll(async ({ browser }) => {
      // Setup: auth + navigate to osd overview page
      const setup = await setupTestSuite(browser, 'overview/osd');

      sharedContext = setup.context;
      sharedPage = setup.page;

      // Initialize page objects for this test suite
      osdProductPage = new OsdProductPage(sharedPage);

      // Verify we're on the OSD product page
    ...
    test('Validate Header Section', async () => {
    ...
    test('Validate pricing section card by checking theirs text', async () => {
    ...
    test('Validate Recommended content section', async () => {
