...
test.describe.serial('Rosa hosted cluster (hypershift) - wizard checks and cluster creation tests (OCP-57641)', { tag: ['@smoke', '@hcp'] }, () => {
  ...
  test('Step - Cluster Settings - Select machine pool node type and node count', async () => {
    ...
    await expect(sharedPage.locator(`text=Select a VPC to install your machine pools into your selected region: ${region}`))
      .toBeVisible();
    ...
    await createRosaWizardPage.selectComputeNodeType(clusterProperties.MachinePools[0].InstanceType);
    await createRosaWizardPage.enableAutoScaling();
    await createRosaWizardPage.disabledAutoScaling();
    await createRosaWizardPage.selectComputeNodeCount(clusterProperties.MachinePools[0].NodeCount);
    ...
    await createRosaWizardPage.rootDiskSizeInput().fill(clusterProperties.MachinePools[0].RootDiskSize);
    await createRosaWizardPage.rosaNextButton().click();
  });
  ...
  test('Step - Cluster Settings - CIDR Ranges - CIDR default values', async () => {
    ...
    await expect(createRosaWizardPage.machineCIDRInput()).toHaveValue(clusterProperties.MachineCIDR);
    await expect(createRosaWizardPage.serviceCIDRInput()).toHaveValue(clusterProperties.ServiceCIDR);
    await expect(createRosaWizardPage.podCIDRInput()).toHaveValue(clusterProperties.PodCIDR);
    await expect(createRosaWizardPage.hostPrefixInput()).toHaveValue(clusterProperties.HostPrefix);
    await createRosaWizardPage.rosaNextButton().click();
  });
  ...
  test('Step - Review and create : Networking definitions', async () => {
    ...
    await createRosaWizardPage.isClusterPropertyMatchesValue('Pod CIDR', clusterProperties.PodCIDR);
    await createRosaWizardPage.isClusterPropertyMatchesValue(
      'Host prefix',
  ...
  test('Create cluster and check the installation progress', async () => {
    ...
    await expect(sharedPage.locator('h2')).toContainText('Installing cluster');
    await expect(sharedPage.locator('a')).toContainText('Download OC CLI');
    await expect(sharedPage.locator('text=Cluster creation usually takes 10 minutes to complete'))
      .toBeVisible();
    ...
    await expect(clusterDetailsPage.clusterDomainPrefixLabelValue())
      .toContainText(clusterProperties.DomainPrefix);
    await expect(clusterDetailsPage.clusterControlPlaneTypeLabelValue())
      .toContainText(clusterProperties.ControlPlaneType);
    await expect(clusterDetailsPage.clusterRegionLabelValue()).toContainText(region);
    await expect(clusterDetailsPage.clusterAvailabilityLabelValue())
      .toContainText(clusterProperties.Availability);
    await expect(clusterDetailsPage.clusterInfrastructureAWSaccountLabelValue())
      .toContainText(awsAccountID);
    await expect(clusterDetailsPage.clusterBillingMarketplaceAccountLabelValue())
      .toContainText(awsBillingAccountID);
    await expect(clusterDetailsPage.clusterMachineCIDRLabelValue())
      .toContainText(clusterProperties.MachineCIDR);
    await expect(clusterDetailsPage.clusterServiceCIDRLabelValue())
      .toContainText(clusterProperties.ServiceCIDR);
    await expect(clusterDetailsPage.clusterPodCIDRLabelValue())
      .toContainText(clusterProperties.PodCIDR);
    await expect(clusterDetailsPage.clusterHostPrefixLabelValue())
      .toContainText(clusterProperties.HostPrefix.replace('/', ''));
  });
