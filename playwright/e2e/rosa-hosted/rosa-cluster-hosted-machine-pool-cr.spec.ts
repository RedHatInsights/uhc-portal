...
const clusterProperties = clusterProfiles['rosa-hosted-public-advanced']['day1-profile'].ClusterName;
...
test.describe.serial(
  ...
  () => {
    ...
    const testMachinePools = [
      ...
      {
        id: `mp-cronly-${Math.random().toString(36).slice(2, 6)}`,
        preference: 'CR only',
        reservationId: 'N/A',
      },
      {
        id: `mp-cronly-${Math.random().toString(36).slice(2, 6)}`,
        preference: 'CR only',
        reservationId: qeInfrastructure.CAPACITY_RESERVATION?.ID || 'N/A',
      },
    ];
    ...
    for (const mp of testMachinePools) {
      test(`Create machine pool with ${mp.preference} preference and ${mp.reservationId} reservation ID`, async ({
        ...
        await machinePoolsPage.selectPrivateSubnet(
          qeInfrastructure.SUBNETS.ZONES[qeInfrastructure.CAPACITY_RESERVATION.AvailabilityZone].PRIVATE_SUBNET_NAME,
        );
    ...
    test.afterAll(async ({ machinePoolsPage }) => {
      for (const mp of testMachinePools) {
        try {
          ...
          console.error(`Failed to delete machine pool ${mp.id}:`, e);
        }
      }
    });
  },
