 ...
 */
export class ClusterCloudTabPage extends BasePage {
  ...
  async checkMainHeader(): Promise<void> {
    await expect(
  ...
  async checkRunItYourselfButton(buttonText: string, expectedUrl: string): Promise<void> {
    const button = this.runItYourselfSection.locator('a').filter({ hasText: buttonText });
    await expect(button).toBeVisible();
    await expect(button).toHaveAttribute('href', expectedUrl);
  }
}
