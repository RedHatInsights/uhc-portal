...
export class ClusterTypesPage extends BasePage {
  ...
  async clickInfrastructureProvider(provider: string): Promise<void> {
    // await this.page.locator('a').filter({ hasText: provider }).click({ force: true });
  ...
  async isAutomated(
    ...
    await expect(learnMoreLink).toHaveAttribute('href', new RegExp(`/installing.*${clusterType}`));
  ...
  async isLocalAgentBased(
    ...
    const expectedHeader = `Install OpenShift on ${clusterHeader} locally ${clusterArch || ''}with Agent`;
    await expect(this.page.locator('h1')).toHaveText(expectedHeader.trim(), { timeout: 30000 });
