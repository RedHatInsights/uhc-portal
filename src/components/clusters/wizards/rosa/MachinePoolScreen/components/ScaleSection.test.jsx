...
// Mock NodeCountInput to prevent prop warnings
jest.mock('~/components/clusters/common/NodeCountInput', () => ({
  __esModule: true,
  default: ({ label }) => {
    const labelText = label || 'Compute node count';
    const inputId = 'node-count-input';
    return (
      <div data-testid="node-count-input">
        <label htmlFor={inputId}>{labelText}</label>
        <input type="number" id={inputId} aria-label={labelText} />
      </div>
    );
  },
}));
