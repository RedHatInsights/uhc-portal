...
describe('when useIsOSDFromGoogleCloud returns true', () => {
  ...
  it('Install to VPC checkbox is checked and disabled with Public cluster privacy', async () => {
    const { user } = render(
      prepareComponent({
        [FieldId.ClusterPrivacy]: ClusterPrivacyType.External,
        [FieldId.GcpAuthType]: GCPAuthType.WorkloadIdentityFederation,
    ...
    // Verify checkbox remains checked and disabled
    await waitFor(() => {
      expect(installToVpcCheckbox).toBeChecked();
    });
    expect(installToVpcCheckbox).toBeDisabled();

    // Click back on Public option
    await user.click(screen.getByRole('radio', { name: /Public/i }));

    // Verify checkbox still remains checked and disabled
    await waitFor(() => {
      expect(installToVpcCheckbox).toBeChecked();
    });
    expect(installToVpcCheckbox).toBeDisabled();
  ...
  it('Install to VPC checkbox is checked and disabled with Private cluster privacy', async () => {
    const { user } = render(
      prepareComponent({
        [FieldId.ClusterPrivacy]: ClusterPrivacyType.Internal,
        [FieldId.GcpAuthType]: GCPAuthType.WorkloadIdentityFederation,
